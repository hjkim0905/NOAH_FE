...
interface HelicopterDeploymentProps {
    isNecessary?: boolean; // 서버에서 오는 값
}

export default function HelicopterDeployment({ isNecessary = true }: HelicopterDeploymentProps) {
    return (
            ...
            <div className="flex items-center justify-between h-full px-[20px]">
                ...
                <div
                    className={`flex items-center justify-center rounded-[100px] ${
                        isNecessary ? 'w-[145px] h-[49.3px] bg-[#FFF0F0]' : 'w-[153px] h-[49px] bg-[#F1F2F5]'
                    }`}
                >
                    <div
                        className={`flex items-center justify-center text-center text-[21px] font-medium leading-[160%] tracking-[-0.02em] ${
                            isNecessary ? 'w-[116px] h-[23.2px] text-[#F44336]' : 'w-[130px] h-[34px] text-[#838E95]'
                        }`}
                        style={{ fontFamily: 'Gotham' }}
                    >
                        {isNecessary ? 'Necessary' : 'Unnecessary'}
                    </div>
                </div>
            </div>
        </div>
